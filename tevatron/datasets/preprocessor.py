...
class EvalPreProcessor:
    def __init__(self, tokenizer, qry_max_length=32, psg_max_length=256, separator=' '):
        self.tokenizer = tokenizer
        self.qry_max_length = qry_max_length
        self.psg_max_length = psg_max_length
        self.separator = separator

    def __call__(self, example):
        docid = example['docid']
        qry_text = example['qry_text']
        qry_text = self.tokenizer.encode(qry_text,
                                     add_special_tokens=False,
                                     max_length=self.qry_max_length,
                                     truncation=True)
        psg_text = example['title'] + self.separator + example['psg_text'] if 'title' in example else example['psg_text']
        psg_text = self.tokenizer.encode(psg_text,
                                     add_special_tokens=False,
                                     max_length=self.psg_max_length,
                                     truncation=True)
        return {'text_id': docid, 'qry_text': qry_text, 'psg_text': psg_text}
